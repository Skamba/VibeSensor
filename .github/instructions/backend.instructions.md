...
- Use the canonical backend command list (defined in the copilot instructions "Canonical commands" section); its backend type gate runs mypy across the `vibesensor` package by default, with only a small temporary denylist for legacy modules that still need dedicated typing cleanup, and `docs/testing.md` covers backend test placement and command details.
