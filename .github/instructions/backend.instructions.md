...
- Backend ownership boundaries:
	...
	- `apps/server/vibesensor/update/`: wheel-based updater package; `manager.py` is the public facade over focused modules for workflow, Wi-Fi, releases, install and rollback, service control, and status. Do not add backward-compatibility shims, static method passthroughs, or module-level aliases in `update/`; when methods move to sub-modules, update callers directly.
- Install: `python -m pip install -e "./apps/server[dev]"` (used by CI).
- Backend type gate: `make typecheck-backend` runs the enforced mypy slice for app/bootstrap, runtime/routes, core typed-boundary modules (`backend_types.py`, `config.py`, `domain_models.py`, `registry.py`, `settings_store.py`, `api_models.py`, `history_{helpers,runs,reports,exports}.py`, `firmware_cache.py`, `gps_speed.py`, `run_context.py`, `runlog.py`, `esp_flash_manager.py`, `json_types.py`, `payload_types.py`, `report_i18n.py`, `ws_models.py`, `order_bands.py`, `peak_classification.py`, `severity.py`, `analysis_persistence.py`, `analysis_settings.py`), and the high-risk `analysis/`, `processing/`, `history_db/`, `metrics_log/`, and `update/` packages.
