...
- Backend type gate: `make typecheck-backend` runs the enforced mypy slice for app/bootstrap, runtime/routes, core typed-boundary modules (`backend_types.py`, `config.py`, `domain_models.py`, `registry.py`, `settings_store.py`, `api_models.py`, `history_{helpers,runs,reports,exports}.py`, `firmware_cache.py`, `gps_speed.py`, `run_context.py`, `json_types.py`, `payload_types.py`), and the high-risk `analysis/`, `processing/`, `history_db/`, and `update/` packages.
