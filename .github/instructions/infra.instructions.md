...
- CI: `.github/workflows/ci.yml` shows steps used in CI (setup Python/Node, pip install -e "./apps/server[dev]", ruff, pytest, UI build).
- Keep CI steps small; if adding new test dependencies, update `apps/server/pyproject.toml` so CI installs them via the editable install.
