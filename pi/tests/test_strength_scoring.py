...
def test_median_even_count_uses_lower_middle() -> None:
    ...
    result = _median([1.0, 2.0, 3.0, 4.0])
...
def test_floor_rms_empty_freq_returns_zero() -> None:
    assert compute_floor_rms(
        freq=[], values=[], peak_indexes=[], exclusion_hz=1.0, min_hz=0, max_hz=100
    ) == 0.0
...
def test_band_rms_center_out_of_range_returns_zero() -> None:
    assert compute_band_rms(freq=[10.0], values=[1.0], center_idx=5, bandwidth_hz=1.0) == 0.0


def test_band_rms_single_bin() -> None:
    result = compute_band_rms(freq=[10.0], values=[3.0], center_idx=0, bandwidth_hz=0.5)
    assert abs(result - 3.0) < 1e-9
...
def test_band_rms_multiple_bins() -> None:
    ...
    result = compute_band_rms(freq=freq, values=values, center_idx=2, bandwidth_hz=1.5)
...
def test_strength_db_equal_band_and_floor() -> None:
    ...
    db = strength_db_above_floor(band_rms=1.0, floor_rms=1.0)
    assert abs(db) < 0.01


def test_strength_db_band_much_above_floor() -> None:
    db = strength_db_above_floor(band_rms=10.0, floor_rms=1.0)
    assert db > 15.0  # ~20 dB


def test_strength_db_floor_zero_returns_finite() -> None:
    db = strength_db_above_floor(band_rms=1e-6, floor_rms=0.0)
    assert db > 0
    assert db < 200
