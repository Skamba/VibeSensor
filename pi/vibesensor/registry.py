...
from .protocol import AckMessage, DataMessage, HelloMessage, client_id_hex
...
class ClientRegistry:
    ...
    def _load_persisted_names(self) -> None:
        ...
        for entry in raw.get("clients", []):
            client_id = str(entry.get("id", "")).lower()
            if len(client_id) == 12:
    ...
    def _get_or_create(self, client_id: str) -> ClientRecord:
        ...
        if record is None:
            ...
            record = ClientRecord(client_id=client_id, name=default_name)
    ...
    def snapshot_for_api(self, now: float | None = None) -> list[dict[str, Any]]:
        ...
        for client_id in sorted(self._clients.keys()):
            record = self._clients[client_id]
            age_ms = int(max(0.0, now_ts - record.last_seen) * 1000) if record.last_seen else None
            rows.append(
                {
                    "id": record.client_id,
                    "name": record.name,
