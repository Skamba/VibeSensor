        ...
        rows.append(
            {
                ...
                "median_vs_run_noise_ratio": float(item.get("median_vs_run_noise_ratio") if item.get("median_vs_run_noise_ratio") is not None else 0.0),
                "p95_vs_run_noise_ratio": float(item.get("p95_vs_run_noise_ratio") if item.get("p95_vs_run_noise_ratio") is not None else 0.0),
                "strength_floor_amp_g": _as_float(item.get("strength_floor_amp_g")),
                "strength_db": _as_float(item.get("strength_db")),
                "presence_ratio": float(item.get("presence_ratio") if item.get("presence_ratio") is not None else 0.0),
                "burstiness": float(item.get("burstiness") if item.get("burstiness") is not None else 0.0),
                "persistence_score": float(item.get("persistence_score") if item.get("persistence_score") is not None else 0.0),
