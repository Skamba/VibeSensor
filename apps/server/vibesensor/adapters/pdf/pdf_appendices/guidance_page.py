...
def draw_capture_guidance_page(c: Canvas, plan: AppendixAPageRenderPlan, title_y: float) -> None:
    ...
    if trace_panel_h <= 20 * mm or not plan.trace_rows:
        return
    _draw_panel(
        c,
        MARGIN,
    ...
    right_y = left_y
    mid = (len(plan.trace_rows) + 1) // 2
    for row in plan.trace_rows[:mid]:
        left_y = (
