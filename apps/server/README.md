...
- `use_cases/run/`: recording orchestration; `lifecycle_state.py` owns the in-memory run session state, `logger.py` owns the coordinator, and `post_analysis.py` owns the background analysis queue above the injected persistence/error boundary.
