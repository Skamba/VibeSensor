...
Plain functions / classes used across multiple test modules.  These live in a
regular module (not ``conftest.py``) so they can be imported unambiguously with
``from _test_helpers import …`` — regardless of whether pytest has already
loaded a sub-directory ``conftest.py`` into ``sys.modules``.

Pytest fixtures (``@pytest.fixture``) remain in ``conftest.py``.
"""
