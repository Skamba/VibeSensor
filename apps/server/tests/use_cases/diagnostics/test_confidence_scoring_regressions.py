...
class TestSuppressEngineAliasesFilterBeforeSlice:
    ...
    def test_valid_finding_not_lost_after_suppression(self) -> None:
        ...
        findings = [
            (
                1.0,
                make_finding(suspected_source="wheel/tire", confidence=0.80, ranking_score=1.0),
            ),
            (
                0.9,
                make_finding(suspected_source="wheel/tire", confidence=0.70, ranking_score=0.9),
            ),
            # These 3 engine findings will be suppressed below threshold
            (
                0.7,
                make_finding(suspected_source="engine", confidence=0.40, ranking_score=0.7),
            ),
            (
                0.6,
                make_finding(suspected_source="engine", confidence=0.38, ranking_score=0.6),
            ),
            (
                0.5,
                make_finding(suspected_source="engine", confidence=0.35, ranking_score=0.5),
            ),
            # These valid findings must NOT be lost
            (
                0.4,
                make_finding(suspected_source="driveline", confidence=0.55, ranking_score=0.4),
            ),
            (
                0.3,
                make_finding(suspected_source="driveline", confidence=0.50, ranking_score=0.3),
            ),
        ]
        result = _suppress_engine_aliases(findings)
        driveline = [f for f in result if str(f.suspected_source) == "driveline"]
    ...
    def test_suppressed_engine_below_threshold_excluded(self) -> None:
        findings = [
            ...
            (
                0.5,
                make_finding(suspected_source="engine", confidence=0.30, ranking_score=0.5),
            ),
        ]
        result = _suppress_engine_aliases(findings)
        engine = [f for f in result if str(f.suspected_source) == "engine"]
