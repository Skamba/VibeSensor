...
def test_wave15_ratio_source_rows_capture_g30_g70_a8_q8_context() -> None:
    ...
    assert "official_540i_xdrive_exact_context" in sources["BMW|5 Series (G30, 2017-2023)"]["sources"]
    assert "official_545e_xdrive_exact_ratios" in sources["BMW|5 Series (G30, 2017-2023)"]["sources"]
    assert "official_i7_edrive50_de_context" in sources["BMW|7 Series (G70, 2023-2026)"]["sources"]
    assert "official_60tfsie_quattro_exact_late_cycle" in sources["Audi|A8 (D5, 2018-2026)"]["sources"]
        ...
        [
            {
                ...
                "reason": "Official BMW Germany catalogs now prove the i7 eDrive50 as the rear-wheel-drive 1-Gang automatic EV with a 245/50 R19 standard tire, but the checked exact sources still do not publish a numeric reduction ratio or overall-drive value for this variant.",
            }
        ],
    )
    _assert_contains_unresolved(
