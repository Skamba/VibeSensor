...
def test_wave17_g60_520i_uses_exact_variant_override() -> None:
    ...
    assert gearbox["gear_ratios"] == pytest.approx([5.25, 3.36, 2.172, 1.72, 1.316, 1.0, 0.822, 0.64])
...
def test_wave17_g32_640i_xdrive_uses_exact_variant_gearbox_override() -> None:
    fastback = resolve_variant(_entry_for("BMW", "6 Series Gran Turismo (G32, 2018-2024)"), "640i xDrive")
...
def test_wave17_ratio_source_rows_capture_bmw_and_audi_context() -> None:
    ...
    assert "official_640i_xdrive_exact_context" in sources["BMW|6 Series Gran Turismo (G32, 2018-2024)"]["sources"]
    assert "official_30tfsi_quattro_early_0b5_context" in sources["Audi|A6 (C7, 2011-2018)"]["sources"]
    assert "official_30tdi_quattro_launch_context" in sources["Audi|A7 Sportback (C7, 2011-2018)"]["sources"]
    ...
    assert {
        ...
        "reason": "Official BMW launch material confirms 18-inch wheels as standard for combustion G60 models, but this pass did not recover a parseable per-variant BMW Germany wheel matrix safe enough to promote exact 19/20/21-inch 520i tire sizes into production data.",
