...
@pytest.fixture
def make_pool():
    """Factory fixture: creates WorkerPools that auto-shutdown after the test."""
    pools: list[WorkerPool] = []

    def _factory(max_workers: int = 2, **kwargs) -> WorkerPool:
        p = WorkerPool(max_workers=max_workers, **kwargs)
        pools.append(p)
        return p
...
class TestWorkerPool:
    ...
    def test_map_unordered_empty(self, make_pool) -> None:
        pool = make_pool()
        assert pool.map_unordered(lambda x: x, []) == {}

    def test_map_unordered_error_handling(self, make_pool) -> None:
    ...
    def test_shutdown_idempotent(self, make_pool) -> None:
...
class TestParallelComputeAll:
    ...
    def test_compute_all_empty_clients(self, make_pool) -> None:
