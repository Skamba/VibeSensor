...
def test_constants_used_for_speed_conversion() -> None:
    ...
    from vibesensor.infra.config.constants import KMH_TO_MPS, MPS_TO_KMH
...
def test_constants_used_for_peak_detection() -> None:
    ...
    from vibesensor.infra.config.constants import PEAK_BANDWIDTH_HZ, PEAK_SEPARATION_HZ
    from vibesensor.vibration_strength import compute_vibration_strength_db
...
def test_silence_db_constant() -> None:
    ...
    from vibesensor.infra.config.constants import SILENCE_DB
...
def test_config_preflight_no_removed_fields() -> None:
    ...
    root = REPO_ROOT
    preflight_path = root / "apps" / "server" / "vibesensor" / "cli" / "config_preflight.py"
...
def test_wheel_hz_and_engine_rpm_single_source() -> None:
    ...
    root = SERVER_ROOT
    files_to_check = [
        root / "vibesensor" / "infra" / "metrics" / "sample_builder.py",
        root / "vibesensor" / "infra" / "metrics" / "logger.py",
...
def test_protocol_docs_match_generated_contract_reference() -> None:
    ...
    from vibesensor.adapters.http.contract_reference_doc import render_contract_reference_markdown
...
def test_sanitize_settings_is_single_source() -> None:
    ...
    from vibesensor.shared.types.backend import sanitize_aspects
