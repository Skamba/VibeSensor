...
from vibesensor.infra.metrics.logger import (
    _MAX_HISTORY_CREATE_RETRIES,
    _RETRY_COOLDOWN_BASE_S,
    RunRecorder,
)
from vibesensor.infra.metrics.post_analysis import _WARN_QUEUE_DEPTH, PostAnalysisWorker
...
class TestRetryCooldown:
    def test_retries_after_cooldown_expires(self, make_logger) -> None:
        ...
        with patch("vibesensor.infra.metrics.logger.time") as mock_time:
...
class TestQueueDepthWarning:
    def test_warning_logged_at_threshold(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(logging.WARNING, logger="vibesensor.infra.metrics.post_analysis"):
