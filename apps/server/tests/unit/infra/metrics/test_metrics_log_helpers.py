...
from vibesensor.infra.metrics.post_analysis import PostAnalysisHealthSnapshot
from vibesensor.infra.metrics.sample_builder import safe_metric
...
def test_append_records_does_not_timeout_on_brief_gap(
    ...
    start_time_utc = snapshot.start_time_utc
    start_mono = snapshot.start_mono_s
    monkeypatch.setattr("vibesensor.infra.metrics.logger.time.monotonic", lambda: 100.0)
...
def test_post_analysis_caps_sample_count_and_stores_sampling_metadata(
    ...
    cap = 200
    monkeypatch.setattr("vibesensor.infra.metrics.post_analysis._MAX_POST_ANALYSIS_SAMPLES", cap)
...
async def test_run_offloads_append_records_with_to_thread(
    ...
    monkeypatch.setattr("vibesensor.infra.metrics.logger.asyncio.to_thread", _fake_to_thread)
    monkeypatch.setattr("vibesensor.infra.metrics.logger.asyncio.sleep", _cancel_sleep)
