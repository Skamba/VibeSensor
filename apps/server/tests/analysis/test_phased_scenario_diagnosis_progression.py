...
class TestScenario1IdleToSpeedUp:
    def test_correct_corner_identified(self) -> None:
        ...
        fault_samples = build_fault_samples_at_speed(
            speed_kmh=100.0,
            fault_sensor="front-right",
            other_sensors=["front-left", "rear-left", "rear-right"],
            n_samples=40,
...
class TestScenario2StopGoIntermittent:
    ...
    def test_speed_band_covers_50_60(self) -> None:
        samples_50 = build_fault_samples_at_speed(
            speed_kmh=50.0,
            fault_sensor="rear-left",
            other_sensors=["front-left", "front-right", "rear-right"],
            n_samples=20,
        ...
        samples_60 = build_fault_samples_at_speed(
            speed_kmh=60.0,
            fault_sensor="rear-left",
            other_sensors=["front-left", "front-right", "rear-right"],
            n_samples=20,
...
class TestScenario3HighwayRearRight:
    def test_correct_corner_rear_right(self) -> None:
        all_samples = build_fault_samples_at_speed(
            speed_kmh=60.0,
        ...
        assert_finding_location(
            summarize_run_data(standard_metadata(), all_samples, include_samples=False),
            "rear-right",
            "Scenario 3",
        )
...
class TestScenario4CoastDownMidRange:
    ...
    def test_correct_corner_front_left(self) -> None:
        assert_finding_location(
            summarize_run_data(
                standard_metadata(),
                self.build_coast_down_samples(add_harmonic=True),
                include_samples=False,
            ),
            "front-left",
            "Scenario 4",
...
class TestScenario5MixedNoiseThenFault:
    def test_correct_corner_front_left(self) -> None:
        ...
        assert_finding_location(
            summarize_run_data(meta, noise_samples + fault, include_samples=False),
            "front-left",
            "Scenario 5",
        )
