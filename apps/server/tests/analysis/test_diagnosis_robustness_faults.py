...
class TestDualFaultTwoCorners:
    ...
    def test_dual_fault_front_right_and_rear_left(self) -> None:
        ...
        for i in range(40):
            ...
            for sensor in ALL_SENSORS:
                if sensor == "front-right":
                    ...
                    vib_db = 8.0
                samples.append(make_sample(t_s=t, speed_kmh=80.0, client_name=sensor, top_peaks=peaks, vibration_strength_db=vib_db, strength_floor_amp_g=0.003))

        summary = summarize_run_data(standard_metadata(), samples, lang="en", file_name="dual_fault_test")
        assert_summary_sections(summary, min_findings=1, min_top_causes=1)
        findings = [
            f for f in summary.get("findings", [])
            if isinstance(f, dict) and not str(f.get("finding_id", "")).startswith("REF_") and float(f.get("confidence_0_to_1") or 0) > 0.10
        ]
        locations = {str(f.get("strongest_location") or "").lower() for f in findings if str(f.get("strongest_location") or "")}
        ...
        intensity_locs = {str(row.get("location", "")).lower() for row in summary.get("sensor_intensity_by_location", [])[:4]}
        for corner in {"front-right", "rear-left"}:
            assert any(corner in loc for loc in intensity_locs)
        if summary.get("top_causes"):
            assert_top_cause_contract(summary["top_causes"][0], expected_source="wheel", confidence_range=(0.15, 1.0))
        ...
        for i in range(35):
            ...
            for sensor in ALL_SENSORS:
                if sensor == "front-left":
                    ...
                    peaks = [{"hz": 142.5, "amp": 0.003}]
                    vib_db = 7.0
                samples.append(make_sample(t_s=t, speed_kmh=90.0, client_name=sensor, top_peaks=peaks, vibration_strength_db=vib_db, strength_floor_amp_g=0.003))

        intensities = summarize_run_data(standard_metadata(), samples, lang="en", file_name="dual_fault_intensity").get("sensor_intensity_by_location", [])
...
class TestClippedSaturatedData:
    def test_saturated_samples_do_not_produce_nan_report(self) -> None:
        ...
        summary = summarize_run_data(standard_metadata(), samples, lang="en", file_name="saturated_test")
        for top_cause in summary.get("top_causes", []):
            assert not math.isnan(top_cause.get("confidence", 0))
        for finding in [f for f in summary.get("findings", []) if isinstance(f, dict) and not str(f.get("finding_id", "")).startswith("REF_")]:
    ...
    def test_clipped_waveform_still_detects_fault_location(self) -> None:
        ...
        for i in range(25):
            for sensor in ALL_SENSORS:
                if sensor == "rear-left":
                    peaks = [{"hz": whz, "amp": 1.5}, {"hz": whz * 2, "amp": 0.6}, {"hz": whz * 3, "amp": 0.3}]
                    vib_db = 50.0
                else:
                    peaks = [{"hz": 142.5, "amp": 0.004}]
                    vib_db = 8.0
        ...
        top_causes = summarize_run_data(standard_metadata(), samples, lang="en", file_name="clipped_location").get("top_causes", [])
        assert_top_cause_contract(top_causes[0], expected_source="wheel", expected_location="rear-left", confidence_range=(0.10, 1.0))
...
class TestDuplicateReplayedSamples:
        ...
        for i in range(30):
            for sensor in ALL_SENSORS:
                if sensor == "front-right":
                    ...
                    peaks = [{"hz": 142.5, "amp": 0.003}]
                    vib_db = 8.0
                current_sample = make_sample(t_s=float(i), speed_kmh=80.0, client_name=sensor, top_peaks=peaks, vibration_strength_db=vib_db, strength_floor_amp_g=0.003)
    ...
    def test_exact_duplicate_samples_do_not_inflate_confidence(self) -> None:
        baseline = summarize_run_data(standard_metadata(), self.fault_scenario_samples(duplicate=False), lang="en", file_name="no_dup_baseline")
        duped = summarize_run_data(standard_metadata(), self.fault_scenario_samples(duplicate=True), lang="en", file_name="dup_test")
        ...
        if baseline_top and duped_top:
            assert float(duped_top[0].get("confidence", 0)) <= float(baseline_top[0].get("confidence", 0)) + 0.15