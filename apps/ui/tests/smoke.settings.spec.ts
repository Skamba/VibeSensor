...
test("analysis settings show a field-specific error when a hard limit is exceeded", async ({ page }) => {
  ...
  await expect.poll(() => analysisPutCalls).toBe(0);
  await expect(page.locator("#wheelBandwidthGuidance")).toContainText("Wheel Bandwidth (%) must stay between 0.1% and 100%");
  await expect(page.locator("#analysisGuidanceHelp")).toHaveAttribute("open", "");
  await expect(page.locator("#wheelBandwidthInput")).toHaveAttribute("aria-invalid", "true");
});

test("failed analysis save preserves the draft and explains that saved settings remain active", async ({ page }) => {
  let analysisPutCalls = 0;
  await installCommonRoutes(page, {
    settingsHandler: async (route) => {
      if (requestPath(route).startsWith("/api/settings/cars")) {
  ...
  await expect(page.locator("#analysisSaveFeedback")).toContainText("Analysis save failed");
  await expect(page.locator("#analysisSaveFeedback")).toContainText("previous saved analysis settings remain active");
  await expect(page.locator("#wheelBandwidthInput")).toHaveValue("7.5");
...
test("failed speed-source save keeps the draft and explains which source remains active", async ({ page }) => {
  ...
  await expect(page.locator("#speedSourceSaveFeedback")).toContainText("Speed source save failed");
  await expect(page.locator("#speedSourceSaveFeedback")).toContainText("GPS remains active right now");
  await expect(page.locator('input[name="speedSourceRadio"][value="manual"]')).toBeChecked();
...
test("failed language save reverts the selector and shows an error", async ({ page }) => {
  ...
  await expect(page.locator("#languageFeedback")).toContainText("Language save failed");
  await expect(page.locator("#languageFeedback")).toContainText("English remains active");
  await expect(page.locator("#languageSelect")).toHaveValue("en");
