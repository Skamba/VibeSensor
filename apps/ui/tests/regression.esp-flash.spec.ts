...
test("settings esp flash tab renders lifecycle state and live logs", async ({
  page,
}) => {
  ...
  await installCommonRoutes(page, {
    espFlashHandler: async (route) => {
      ...
      if (path === "/api/esp-flash/logs") {
        ...
        await fulfillJson(route, {
          from_index: after,
          next_index: logCursor,
          lines: logs.slice(after, logCursor),
        });
        return;
      }
  ...
  const pairedLayout = await page.evaluate(() => {
    const journeyCard = document
      .querySelector("#espFlashJourneyPanel")
      ?.closest(".maintenance-card");
    const logCard = document
      .querySelector("#espFlashLogPanel")
      ?.closest(".maintenance-card");
    const readinessCard = document
      .querySelector("#espFlashReadinessPanel")
      ?.closest(".maintenance-card");
    const summaryCard = document
      .querySelector("#espFlashStartSummary")
      ?.closest(".maintenance-card");
    if (
      !(journeyCard instanceof HTMLElement) ||
      !(logCard instanceof HTMLElement)
    ) {
      return null;
    }
  ...
  await expect(page.locator("#espFlashReadinessPanel")).not.toContainText(
    "Expected stages",
  );
  await expect(page.locator("#espFlashJourneyPanel")).toContainText(
    "Write firmware",
  );
  await expect(
    page.locator(
      '#espFlashJourneyPanel .maintenance-stage[aria-current="step"]',
    ),
  ).toHaveAttribute("data-stage-phase", "flashing");
  await expect(
    page.locator(
  ...
  await expect(
    page.locator(
      '#espFlashJourneyPanel .maintenance-stage[data-stage-phase="validating"] .maintenance-stage__marker',
    ),
  ).toHaveText("✓");
...
test("settings esp flash status falls back to idle when API omits state", async ({
  page,
}) => {
  await installCommonRoutes(page, {
    espFlashHandler: async (route) => {
  ...
  await expect(page.locator("#espFlashReadinessPanel")).toContainText(
    "No serial device is detected yet",
  );
  await expect(page.locator("#espFlashTab")).not.toContainText(
    "Current readiness",
  );
  await expect(page.locator("#espFlashJourneyPanel")).toContainText(
    "Validate board and bundle",
  );
  await expect(page.locator("#espFlashTab")).toContainText("What happens next");
  await expect(page.locator("#espFlashLogPanel")).toContainText(
    "No active flash log",
  ...
  await expect(page.locator("#espFlashTab")).toContainText(
    "Starting a flash builds the latest firmware",
  );
});

test("settings esp flash failure shows retry guidance, retained failed stage, and fallback attempt history", async ({
  page,
}) => {
  await installCommonRoutes(page, {
    espFlashHandler: async (route) => {
  ...
  await expect(page.locator("#espFlashStartSummary")).toContainText(
    "Reconnect and retry the upload",
  );
  await expect(page.locator("#espFlashStartSummary")).toContainText(
  ...
  await expect(
    page.locator(
      '#espFlashJourneyPanel .maintenance-stage[data-stage-phase="flashing"]',
    ),
