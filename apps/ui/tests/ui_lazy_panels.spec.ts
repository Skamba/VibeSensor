...
test.describe("createLazyUiPanels", () => {
  ...
  test("prefetchHiddenPanels schedules lazy view preloads through the deferred scheduler", () => {
    let historyPreloads = 0;
    let scheduledTask: (() => void) | null = null;
    let settingsPreloads = 0;
    ...
    expect(historyPreloads).toBe(0);
    expect(settingsPreloads).toBe(0);
    expect(scheduledTask).not.toBeNull();

    scheduledTask?.();
    expect(historyPreloads).toBe(1);
    expect(settingsPreloads).toBe(1);

    scheduledTask = null;
    lazyPanels.prefetchHiddenPanels();
    scheduledTask?.();
    expect(historyPreloads).toBe(2);
    expect(settingsPreloads).toBe(2);
  });

  test("prefetchHiddenPanels prefers requestIdleCallback when available", () => {
    let historyPreloads = 0;
