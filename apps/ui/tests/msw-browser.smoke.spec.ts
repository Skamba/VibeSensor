...
test("browser MSW mock mode boots history and settings without a backend", async ({
  ...
  await expect(page.locator('[data-run-toggle="details"][data-run="run-001"]')).toBeVisible();

  await openSettingsTab(page, "carTab");
  await expect(page.locator('[data-settings-tab="carTab"]')).toHaveAttribute("aria-selected", "true");
