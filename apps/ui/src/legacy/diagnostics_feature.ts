...
export function createDiagnosticsFeature(ctx) {
  const { state, els, t, fmt, locationCodeForClient, carMapPositions, carMapWindowMs, metricField } = ctx;
  ...
  function pushCarMapSample(byLocation) {
    const now = Date.now();
  ...
  function carMapIntensityByLocation() {
  ...
  function triggerCarMapPulse(locationCodes) {
  ...
  function extractLiveLocationIntensity() {
    const byLocation = {};
  ...
  function bindMatrixTooltips() {
    ...
    for (const cell of cells) {
      ...
      cell.addEventListener("mouseenter", (ev) => showMatrixTooltip(tooltipForCell(sourceKey, severityKey), ev.clientX, ev.clientY));
      cell.addEventListener("mousemove", (ev) => showMatrixTooltip(tooltipForCell(sourceKey, severityKey), ev.clientX, ev.clientY));
  ...
  function renderMatrix() {
    ...
    const bodyRows = [...state.strengthBands].sort((a, b) => b.min_db - a.min_db).map((band) => {
      ...
      return `<tr><td>${ctx.escapeHtml(t(band.labelKey))}</td>${cells}</tr>`;
    }).join("");
  ...
  function ensureStrengthChart() {
    ...
    const shadePlugin = { hooks: { drawClear: [
      (u) => {
        const ctx2 = u.ctx;
  ...
  function applyServerDiagnostics(diagnostics, hasFreshFrames = false) {
    ...
    const eventPulseLocations = [];
    if (events.length) {
      for (const ev of events.slice(0, 6)) {
        const labels = Array.isArray(ev.sensor_labels) ? ev.sensor_labels.join(", ") : (ev.sensor_label || "--");
        pushVibrationMessage(`Strength ${String(ev.severity_key || "l1").toUpperCase()} (${fmt(ev.vibration_strength_db || 0, 1)} dB) @ ${fmt(ev.peak_hz || 0, 2)} Hz | ${labels} | ${ev.class_key || "other"}`);
