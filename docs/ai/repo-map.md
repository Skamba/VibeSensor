...
- `adapters/pdf/`: PDF/report rendering pipeline and report mapping entrypoints. `report_context.py` owns context assembly (data-prep, card-assembly, domain aggregate reconstruction) and bridges `use_cases/history/report_interpretation`; `mapping.py` is the thin mapper (context → `ReportTemplateData`) and does not import from `use_cases/`; `pdf_page1.py` and `pdf_page2.py` are thin page composers over focused `_panel_*.py` section modules.
