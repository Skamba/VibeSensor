...
- Indirect references/checks: `tools/tests/heuristics_audit.py`, `apps/server/tests/hygiene/test_ai_smoke.py`
...
4. Added test guard:
   - `apps/server/tests/hygiene/test_ai_smoke.py` now asserts presence of SSH first-boot validation logic in build wrapper.
