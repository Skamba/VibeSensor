...
| Installable backend package / wheel compatibility | The backend package declares a minimum of `>=3.13` in [`apps/server/pyproject.toml`](../apps/server/pyproject.toml). | Not applicable. | This is the compatibility floor for the packaged server, not the exact native-dev or CI pin. |
| Manual Raspberry Pi install and on-device runtime | Support follows the Raspberry Pi OS Lite (Trixie) baseline and the `python3`, `python3-venv`, and `python3-pip` packages installed by [`apps/server/scripts/install_pi.sh`](../apps/server/scripts/install_pi.sh). | Not required on-device. | The Pi runtime path is OS-baseline-driven rather than tied to the exact native-dev pin in `.python-version`. |
...
3. If backend package compatibility changes independently from the native pin,
   update [`apps/server/pyproject.toml`](../apps/server/pyproject.toml) and any
   docs that describe the packaged-server compatibility floor.
