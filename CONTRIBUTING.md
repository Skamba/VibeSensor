...
Shared UI constants in `apps/ui/src/constants.ts` are **not** generated by this flow. Drift for `LOCATION_CODES` and `METRIC_FIELDS` is guarded by backend hygiene tests instead.
...
- UI contract drift: run `cd apps/ui && npm run sync:contracts` to regenerate the generated HTTP API and WS contract files. Shared UI constants in `apps/ui/src/constants.ts` are checked by backend hygiene tests.
